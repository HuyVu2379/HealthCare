...
public class HealthRecordEventConsumer {
    private final MedicalHistoryRepository medicalHistoryRepository;
    private final PatientRepository patientRepository;
    private final DoctorRepository doctorRepository;
    private final AppointmentClient appointmentClient;
    @RabbitListener(queues = "HEALTH_RECORD_USER_NOTIFICATION_QUEUE")
    public void handleAppointmentEvent(MedicalRecordPayload message) {
        ...
        try{
            if(message.getEventType().equals("MEDICAL_RECORD_CREATED")) {
                Patient patient = patientRepository.findById(appointmentDetails.getPatientId()).orElse(null);
                Doctor doctor = doctorRepository.findById(appointmentDetails.getDoctorId()).orElse(null);
                if (patient == null || doctor == null) {
                    log.error("Patient or Doctor not found for appointment ID: {}", message.getAppointmentId());
                    return;
                }
